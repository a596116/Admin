<template>
  <main class="mx-1 mb-1 bg-hd-bg" v-show="menuStore.historyMenus.length">
    <div
      ...
      <div
        ...
        :key="index"
        class="box-border flex items-center px-2 py-1 text-sm duration-300 border rounded-lg shadow-sm hover:bg-hd-primary group"
        :class="{
          'bg-hd-primary text-white': route.name === link.route,
          'bg-hd-bg-1 text-hd-white': route.name !== link.route,
        }"
        ...
        <svg-icon
          name="close"
          class="w-4 h-4 ml-1 duration-150 cursor-pointer hover:text-hd-primary-hover"
